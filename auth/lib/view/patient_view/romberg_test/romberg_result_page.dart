...
class RombergResultPage extends StatefulWidget {
  final TestResult result;
  const RombergResultPage({super.key, required this.result});

  @override
  State<RombergResultPage> createState() => _RombergResultPageState();
}

class _RombergResultPageState extends State<RombergResultPage> {
  ...
  Future<void> _sendTestResultDirectly() async {
    ...
    try {
      ...
      var hipSway = _safeGet(['hip_sway_signal', 'hip_sway', 'Romberg_HipSway']);
      var shoulderTilt = _safeGet(['shoulder_tilt_signal', 'shoulder_tilt', 'Romberg_ShoulderTilt']);
      ...
      await FirebaseFirestore.instance.collection('evaluations').doc(user.uid).set({
        ...
        'testResults': {
          'Romberg_Prediction': widget.result.prediction ?? widget.result.label ?? "UNKNOWN",
          'Romberg_HipSway': hipSway,
          'Romberg_ShoulderTilt': shoulderTilt,
        }
      ...
      await FirebaseFirestore.instance.collection('notifications').add({
        'doctorUid': doctorUid,
        'patientName': patientName,
        'message': 'أتم المريض $patientName اختبار الاتزان (Romberg Test) بنجاح.',
        'time': FieldValue.serverTimestamp(),
        'isRead': false,
      });

      await FirebaseFirestore.instance.collection('profiles').doc(user.uid).update({
        'testsStatus.Balance': false,
      });
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(title: const Text('نتيجة اختبار الاتزان'), centerTitle: true),
        body: ListView(
          ...
          children: [
            Container(
              ...
              child: Column(
                children: [
                  ...
                  Text(widget.result.hasError ? 'تعذر إكمال التحليل' : (isHealthy ? 'الاتزان طبيعي وثابت' : 'تم رصد اضطراب أو ترنح في الثبات الحركي'), textAlign: TextAlign.center, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
                  const SizedBox(height: 18),
                  Row(
                    children: [
                      Expanded(child: Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xFFF4FAFF), borderRadius: BorderRadius.circular(18)), child: Column(children: [const Text('درجة الثبات', style: TextStyle(color: Colors.grey, fontSize: 13)), const SizedBox(height: 6), Text(widget.result.score?.toStringAsFixed(2) ?? '-', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900))]))),
            ...
            Container(
              ...
              child: Text(hasConcern ? 'سيتم مشاركة تقرير درجات التمايل (Sway) تلقائياً مع طبيبك المختص للمتابعة.' : 'نتيجة اختبار الاتزان مطمئنة ومستقرة.', textAlign: TextAlign.center, style: TextStyle(color: hasConcern ? const Color(0xFF9A5B00) : const Color(0xFF087443), fontWeight: FontWeight.w800, height: 1.6)),
            ),
