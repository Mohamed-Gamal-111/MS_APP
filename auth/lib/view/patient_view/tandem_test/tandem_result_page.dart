...
class TandemResultPage extends StatefulWidget {
  final TestResult result;
  const TandemResultPage({super.key, required this.result});

  @override
  State<TandemResultPage> createState() => _TandemResultPageState();
}

class _TandemResultPageState extends State<TandemResultPage> {
  ...
  dynamic _safeGet(List<String> keys) {
    for (var k in keys) {
      if (widget.result.features.containsKey(k) && widget.result.features[k] != null) {
        return widget.result.features[k];
      }
    }
    return 0.0;
  }
  ...
  Future<void> _sendTestResultDirectly() async {
    ...
    try {
      ...
      var velocity = _safeGet(['velocity_signal', 'Velocity_Std', 'Velocity']);
      var spineTilt = _safeGet(['spine_lateral_signal', 'Spine_Tilt', 'spine_tilt']);
      ...
      await FirebaseFirestore.instance.collection('evaluations').doc(user.uid).set({
        ...
        'testResults': {
          'Tandem_Prediction': widget.result.prediction ?? widget.result.label ?? "UNKNOWN",
          'Tandem_Velocity': velocity,
          'Tandem_SpineTilt': spineTilt,
        }
      ...
      await FirebaseFirestore.instance.collection('notifications').add({
        'doctorUid': doctorUid,
        'patientName': patientName,
        'message': 'أتم المريض $patientName اختبار المشي (Tandem Gait) بنجاح.',
        'time': FieldValue.serverTimestamp(),
        'isRead': false,
      });

      await FirebaseFirestore.instance.collection('profiles').doc(user.uid).update({
        'testsStatus.Walking': false,
      });

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم حفظ النتيجة وإغلاق اختبار المشي بنجاح')));
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(title: const Text('نتيجة اختبار المشي'), centerTitle: true),
        body: ListView(
          ...
          children: [
            Container(
              ...
              child: Column(
                children: [
                  ...
                  Text(widget.result.hasError ? 'تعذر إكمال التحليل الحركي' : (isHealthy ? 'المشي مستقر وبنمط طبيعي' : 'تم رصد اضطراب أو عدم انتظام في الخطوات وعمود الظهر'), textAlign: TextAlign.center, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
                  const SizedBox(height: 18),
                  Row(
                    children: [
                      Expanded(child: Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xFFF4FAFF), borderRadius: BorderRadius.circular(18)), child: Column(children: [const Text('مؤشر جودة المشي', style: TextStyle(color: Colors.grey, fontSize: 13)), const SizedBox(height: 6), Text(widget.result.score?.toStringAsFixed(2) ?? '-', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900))]))),
            ...
            Container(
              ...
              child: Text(hasConcern ? 'سيتم إرسال حسابات انحناء العمود الفقري وسرعة الخطوات تلقائياً لطبيبك.' : 'مؤشرات نمط المشي مطمئنة ومستقرة.', textAlign: TextAlign.center, style: TextStyle(color: hasConcern ? const Color(0xFF9A5B00) : const Color(0xFF087443), fontWeight: FontWeight.w800, height: 1.6)),
            ),
