...
import 'tandem_result_page.dart';

class TandemUploadPage extends StatefulWidget {
  const TandemUploadPage({super.key});

  @override
  State<TandemUploadPage> createState() => _TandemUploadPageState();
}

class _TandemUploadPageState extends State<TandemUploadPage> {
  PlatformFile? video;
  bool loading = false;
  String? error;
  ...
  Future<void> upload() async {
    if (video == null) {
      setState(() => error = 'من فضلك قم باختيار فيديو اختبار السير والمشي أولاً.');
      return;
    }
    ...
    try {
      ...
      Map<String, dynamic> existingTestResults = {};
      if (docSnapshot.exists && docSnapshot.data()?['testResults'] != null) {
        existingTestResults = Map<String, dynamic>.from(docSnapshot.data()?['testResults']);
      }

      existingTestResults['Tandem_Prediction'] = result.prediction ?? "UNKNOWN";

      if (!docSnapshot.exists) {
        await docRef.set({
          'doctorUid': '4PM43A2ahfR6fCZ1mlHTkaQIJXZ2', // توحيد الـ UID الحقيقي المعتمد لدكتور صلاح
          'patientUid': currentUser!.uid,
          'patientName': currentUser!.displayName ?? "Sara", // توحيد الاسم الافتراضي Sara
      ...
      await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => TandemResultPage(result: result),
        ),
      );
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: const Text('اختبار السير والمشي المستقيم'),
          centerTitle: true,
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [
                    BoxShadow(
                      color: Colors.black.withValues(alpha: 0.06), // حماية من الـ Deprecation Warning
                      blurRadius: 18,
                    )
                  ],
                ),
                child: Column(
                  ...
                  children: [
                    const Icon(Icons.directions_walk, size: 70, color: Color(0xFF1E9BEF)),
                    const SizedBox(height: 18),
                    const Text(
                      'اختبار جيت للـمشي (Tandem Gait Test)',
                      textAlign: TextAlign.center,
                      style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, height: 1.3),
                    ),
                    const SizedBox(height: 12),
                    const Text(
                      'قم بتصوير المريض أثناء المشي في خط مستقيم متزن لعدة خطوات ليقوم النظام بحساب ثبات الأطراف وعمود الظهر.',
                      textAlign: TextAlign.center,
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton.icon(
                          style: ElevatedButton.styleFrom(
                            backgroundColor: const Color(0xFF1E9BEF), // الزرار الأزرق الصريح والعريض الموحد
                    ...
                    if (error != null) ...[
                      ...
                      Container(
                        ...
                        child: Row(
                          crossAxisAlignment: CrossAxisAlignment.start, // الحماية الكاملة لـ Parameter الـ Syntax
                          children: [
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    hasVideo ? selectedVideo!.name : 'اختر مقطع فيديو فحص السير والمشي',
                    maxLines: 1,
                  ...
                  Text(
                    hasVideo ? 'تم إرفاق ملف الفحص بنجاح $fileSizeText' : 'اضغط هنا لرفع فيديو السير المسجل من الاستوديو',
...
class _WaitingForVideoBox extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: const Text(
        'بمجرد اختيار ملف الفيديو، سيظهر لك زر بدء المعالجة والتحليل فورا.',
        textAlign: TextAlign.center,
    ...
    return Container(
      ...
      child: const Column(
        children: [
          ...
          SizedBox(height: 14),
          Text(
            'جاري استخراج سرعة الخطوات وحساب زوايا ميلان الظهر والجذع الحركي...',
            textAlign: TextAlign.center,
